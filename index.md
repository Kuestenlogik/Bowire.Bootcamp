---
_layout: landing
summary: 'A hands-on tutorial that takes you from your first API call to building, mocking, AI-integrating and shipping protocol plugins for Bowire, Küstenlogik''s multi-protocol API workbench.'
---

<div class="bowire-docs-hero">
  <svg class="bowire-docs-hero-logo standalone-only" viewBox="0 0 261.48001 154.57416" overflow="visible" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-label="Bowire">
    <!-- 2026-06: the original inline SVG concatenated the four canonical
         paths from images/bowire_logo.svg into a single compound path with
         relative `m` continuations. The compound rendered fine in the
         browser but Chromium's PDF rasterizer dropped the inner B + the
         brand-mark circle from the print pass, leaving only the outer
         knot. Reverted to the 4-path structure from the source SVG
         (path85 = knot, path86 + path89 = B lobes, path87 = brand circle)
         so each shape fills independently — no fill-rule winding to
         resolve, no compound-path edge cases for the PDF backend.
         Source: images/bowire_logo.svg -->
  ...
  <h1 class="bowire-docs-hero-title standalone-only">Bowire Bootcamp</h1>
  <h1 class="bowire-docs-hero-title online-only">Bootcamp</h1>
  <p class="bowire-docs-hero-tagline">
    Welcome to the Bowire Bootcamp — a hands-on tutorial that takes you from your first API call to building, mocking, AI-integrating and shipping protocol plugins for <a href="https://bowire.io">Bowire</a>, Küstenlogik's multi-protocol API workbench.
  </p>
</div>
