...
> **Read a clean result carefully.** Four of those five rules inspect the *response* shape, and how much of it discovery produces differs by protocol. Against gRPC the descriptors carry full message types and every rule evaluates; against REST, the response shape comes from the operation's 2xx JSON schema (or, embedded, the endpoint's declared response type) — an endpoint that declares neither has no response shape for those four rules to read. Since v2.8 the report says so: a note under the summary names how many methods it could not evaluate and why. Read "no findings" together with that note — see [Design-time lint](https://bowire.io/docs/features/lint.html).
